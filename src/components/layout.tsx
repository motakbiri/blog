...
const Layout = ({ location, title, children, type }) => {
  ...
  const isBlogList = type === 'blog-list'
  const [mentorSource, setMentorSource] = React.useState('')
  const toggle = (
    <ThemeToggler>
      {({ toggleTheme, theme }) => {
        const isDarkMode = theme === 'dark'
        setMentorSource(!isDarkMode ? 'https://cdn.mentorcruise.com/img/banner/find-me-on-mc.jpg' : 'https://cdn.mentorcruise.com/img/banner/find-me-on-mc-dark.jpg') 
        if (theme == null) {
          return null
        }
      ...
      <div className="grid grid-cols-10 gap-4 max-w-screen-md lg:max-w-screen-lg m-auto font-mont">
        ...
        <p className="hidden font-medium font-inter pt-6 sm:block">
          <Link
            ...
            target="_blank"
            to={'https://linkedin.com/in/takbiri'}
          >
            About
          </Link>
        </p>
        <p className="hidden font-medium font-inter pt-6 sm:block">
          <Link
            ...
            target="_blank"
            to={'https://twitter.com/mohammadtkbr'}
          >
            Contact
          </Link>
        </p>
  ...
  const recentBlogs = (
    <div className="relative py-4 sm:max-w-3xl sm:mx-auto mb-10">
  ...
  return (
    <div
      className="container mx-auto transition duration-200 ease-out h-full"
      style={{
    ...
    >
      {/* <div className="sidebar">
        ...
        <main style={{height: isBlogList ? '100vh' : '100%'}}>
        {(function() {
        switch (type) {
          case 'root':
            return (
          ...
          <Footer mentorSource={mentorSource}/>
        </main>
      </div>
  )
}
